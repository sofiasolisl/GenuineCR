...
class Route:
    ...
    def getByLocationsId (cls,data):
        ...
        if len(result)>0:
            ...
            return None
    @classmethod
    def getByFromId (cls,from_id):
        query=f"SELECT r.id, l1.id as from_location_id, l1.location as from_location,l2.id as to_location_id, l2.location as to_location from routes r LEFT JOIN locations l1 ON l1.id=r.from_location_id LEFT JOIN locations l2 ON l2.id=r.to_location_id WHERE from_location_id = {from_id}"
    ...
    def getFullRoutes(cls):
        query='SELECT r.id, r.from_location_id,r.to_location_id, l1.id as from_location_id, l1.location as from_location,l2.id as to_location_id, l2.location as to_location from routes r LEFT JOIN locations l1 ON l1.id=r.from_location_id LEFT JOIN locations l2 ON l2.id=r.to_location_id;'
        results=connectToMySQL('oaerflmy_crauthentic').query_db(query)
        routes=[]
        for route in results:
            routes.append(cls(route))
    ...
    def getFullRoutesById(cls,id):
        query=f'SELECT r.id, r.from_location_id,r.to_location_id, l1.id as from_location_id, l1.location as from_location,l2.id as to_location_id, l2.location as to_location from routes r LEFT JOIN locations l1 ON l1.id=r.from_location_id LEFT JOIN locations l2 ON l2.id=r.to_location_id WHERE r.id = {id};'
        result=connectToMySQL('oaerflmy_crauthentic').query_db(query)
        #print("Result", result)
        if len(result)>0:
            return cls(result[0])
